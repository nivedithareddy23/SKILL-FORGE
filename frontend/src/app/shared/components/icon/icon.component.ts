...
@Component({
  selector: 'app-icon',
  standalone: true,
  template: `<span class="icon-wrap" [style.width.px]="size" [style.height.px]="size" [innerHTML]="safeHtml"></span>`,
  styles: [`:host { display: inline-flex; align-items: center; } .icon-wrap { display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; } .icon-wrap ::ng-deep svg { width: 100%; height: 100%; }`]
})
export class IconComponent {
  private sanitizer = inject(DomSanitizer);

  @Input() set name(v: string) { this._name = v; this._updateHtml(); }
  @Input() set size(v: number) { this._size = v; this._updateHtml(); }
  ...
  private _updateHtml(): void {
    this.safeHtml = this.sanitizer.bypassSecurityTrustHtml(this.getSvg());
  }
  ...
  private icons: Record<string, string> = {
    ...
    bar_chart:  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>`,
  ...
  private getSvg(): string {
    const svg = this.icons[this._name] ?? this.icons['info'];
    // Inject width/height via style so it scales with the size input
    return svg.replace('<svg ', `<svg width="${this._size}" height="${this._size}" `);
  }
}